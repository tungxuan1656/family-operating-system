...
- Local self-review: `./frontend/scrips/review-check.sh self-review [base_ref]` (alias: `local`)
- PR review: `./frontend/scrips/review-check.sh review-pr <PR_NUMBER>` (alias: `pr`)
...
2. `./frontend/scrips/review-check.sh self-review [base_ref]` (alias: `local`)
3. Complete process `P01 -> P09`.
4. `pnpm run lint`
5. Publish review summary with evidence (+ closure table for follow-up).
6. If Gate D/F fails, stop, fix Vietnamese-character findings, and rerun until clean.
